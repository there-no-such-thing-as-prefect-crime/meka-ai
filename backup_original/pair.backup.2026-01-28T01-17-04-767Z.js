...
// ---------------- CONFIG ----------------
const BOT_NAME_FREE = '🤖 M E K A - A I 🤖';
...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/DXasbP5xOeT77AmzaPykEw',
  FREE_IMAGE: 'https://i.ibb.co/8nBz6m19/download-1.jpg',
  NEWSLETTER_JID: '0029Vb7Ew0t8fewhGUdO1J0s@newsletter', // replace with your own newsletter its the main newsletter
  ...
  SUPPORT_NEWSLETTER: {
    jid: '0029Vb7Ew0t8fewhGUdO1J0s@newsletter',  // Your channel
  ...
  DEFAULT_NEWSLETTERS: [
    ...
    { 
      jid: '0029Vb7Ew0t8fewhGUdO1J0s@newsletter',  // Your channel
      emojis: ['❤️', '🌟', '🔥', '💯'],
      name: '• meka-ai •', //your channel name or just desplay name
      description: 'Free Channel'
    },
    // Other popular newsletters if u have more
    { 
      jid: '0029Vb7Ew0t8fewhGUdO1J0s@newsletter', 
      emojis: ['🎵', '🎶', '📻'],
      name: 'Music Updates'
    }
  ...
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '2349021506036',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7Ew0t8fewhGUdO1J0s',
  BOT_NAME: '🤖 M E K A - A I 🤖',
  BOT_VERSION: '1.0.0',
  OWNER_NAME: '• mxgamecoder •',
  IMAGE_PATH: 'https://i.ibb.co/LzmhF2Z9/Ouka-Shiunji.jpg',
  BOT_FOOTER: '> ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴍx ᴄᴏᴅᴇʀ',
  BUTTON_IMAGES: { ALIVE: 'https://i.ibb.co/LzmhF2Z9/Ouka-Shiunji.jpg' }
};

// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://mxgamecoderbot:mxgamecoder@cluster0.idpy4.mongodb.net/?appName=Cluster0';
const MONGO_DB = process.env.MONGO_DB || 'meka-ai_db';
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '©₥Ɇ₭₳-₳ł ';
...
╭─「  \`🤖${title}\`  」 ─➤*  
*│
*│*🥷 *Oᴡɴᴇʀ :* ${config.OWNER_NAME || '• mxgamecoder •'}
...
`.trim();
    ...
    const defaultImg = "https://i.ibb.co/LzmhF2Z9/Ouka-Shiunji.jpg";
...
case 'apkfind': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'MEKA-AI';
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FREE, message: '🍬 MEKA-AI', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || '© ▶ • meka-ai • '}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
